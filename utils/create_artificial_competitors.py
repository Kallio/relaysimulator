#!/usr/bin/env python3
"""Generate a synthetic IOF-XML ResultList with artificial relay competitors.

Reads a CourseData XML file to get courses/controls, optionally reads an existing
ResultList for speed calibration, and outputs a fully synthetic ResultList
compatible with simulator.py.

Usage:
    # Non-interactive (all args via CLI)
    python utils/create_artificial_competitors.py \
      --courses data/runners.j2025_ju_iof_fixed_courses.xml \
      --ref-results data/results_j2025_ju_iof_fixed.xml \
      --out data/artificial_jukola.xml --teams 50 --seed 42

    # Interactive (missing args are prompted with educational examples)
    python utils/create_artificial_competitors.py \
      --courses data/runners.j2025_ju_iof_fixed_courses.xml
"""
import argparse
...
FI_CLUBS = [
    "Syntetiset Susiet", "Virtuaali Metsästäjät", "Algoritmi Albatrossit",
    "Binary Bäverit", "Cache Caper", "Digitidaudit", "Dockerit",
    "Elixir Eaglees", "Funktionaaliset Haukat", "Git Gepardit",
    "Haskell Hiirulinnat", "IoT Ilves", "Java Joutsenet",
    "Kubernetes Kettut", "Lambda Leijonat", "Machine Learning Merimetsot",
    "Neural Nets Nääät", "OpenGL Oravit", "Python Pöllöt",
    "Quantum Kvarttit", "Rust Räikeät", "SQL Saliit",
    "TensorFlow Tiikerit", "UDP Ulvovat", "Vector Vihreat",
    "WebSocket Wessut", "XOR Ksot", "Yield Yksisarviset",
    "ZeroMQ Sudet", "Agile Ahmat", "Bitwise Bisamaat",
    "Container Corgit", "Debug Delfiinit", "Ethernet Etanat",
    "Fiber Fluorit", "Grafana Grizzlyt", "Hash Hylkeet",
    "Index Ilmakotkat", "JSON Jänikset", "Kernel Kasuaarit",
    "Load Balancer Linnut", "Mutex Muurahaiset", "Nginx Nault",
    "OAuth Oinas", "Proxy Puumiehet", "Query Kuukseset",
    "Recursion Revontulet", "SSH Sassit", "Thread Tiikurit",
    "Unicode Urut", "VPN Vesikot", "YAML Äyriäiset",
    "Zabbix Zirkat", "Bootsi Bärsärkar", "Cache Kauriinit",
    "Deploy Deliinit", "Fetch Fregatit", "Heap Hirvieläimet",
    "Ingress Iibikset", "Join Joutsenet", "Kill Kärppä",
    "Log Lindut", "Map Mervet", "Node Norpat",
    "Patch Pistesauat", "Queue Kvizelit", "Route Rooft",
    "Stack Stukit", "Tunnel Tupsut", "Update Urut",
    "Vault Visakoira", "Web Varpuset", "Zip Zebrat",
]
...
def parse_coursedata(courses_path):
    ...
    # Parse Control positions (optional, not used for time generation but good to have)
    controls = {}
    ...
    for course in root.findall('.//iof:RaceCourseData/iof:Course', ns):
        ...
        # Extract ordered controls and leg lengths
        ordered_controls = []
        leg_lengths = []
...
def analyze_ref_results(ref_path):
    ...
    stats = {}  # leg -> {'speeds': [...], 'status_counts': {...}}
...
def build_speed_sampler(ref_stats, legs, speed_mode, avg_speed=None, speed_variance=None):
    ...
    Each runner gets a consistent speed factor across all legs."""
    ...
    if not ref_stats:
        # Fallback: no reference data, use reasonable defaults
    ...
    # Compute per-leg speed ranges for mapping percentile -> speed
    leg_speeds = {}
    for leg in sorted(ref_stats.keys()):
        s = sorted(ref_stats[leg]['speeds'])
        if s:
    ...
    def sampler(runner_id, leg, rng):
        ...
        s_min = speeds[0]
        s_max = speeds[-1]
        # Use beta distribution for realistic right-skewed shape
        # Use deterministic seed per runner to ensure consistency
...
# Default course lengths per leg (meters) when CourseData has Length=0
...
def generate_splits(controls, leg_lengths, total_time):
    ...
    Returns list of (control_code, cumulative_time)."""
    if not controls or len(controls) < 2:
        return []

    # controls includes Start + Finish, leg_lengths is between them
    # We want times for each Control (not Start), including Finish
    ...
    # Average segment time, then add noise
    ...
    # Ensure last control matches total_time
    if result:
...
def generate_team_name(bib, rng):
    idx = (bib - 1) % len(FI_CLUBS)
    suffix = ""
    if bib > len(FI_CLUBS):
        suffix = f" {bib}"
    return FI_CLUBS[idx] + suffix
...
def interactive_setup(courses_by_leg, ref_stats, args, is_individual=False):
    """Fill in missing args via interactive prompts."""
    n_legs = max(courses_by_leg.keys()) if courses_by_leg else 7
    print()
    if is_individual:
        event_default = "Synthetic Individual Orienteering Race"
    elif n_legs == 7:
        event_default = "Synthetic Jukolan Viesti"
    else:
        event_default = "Synthetic Venla"

    if not args.event_name:
        args.event_name = prompt("Event name", default=event_default)

    if args.teams is None:
        args.teams = prompt("Number of teams", default=50, type_fn=int)

    if not args.mass_start:
        args.mass_start = prompt("Mass start time", default="2025-06-14T23:00:00+03:00")

    # Show reference stats if available
    if ref_stats:
        print()
    ...
    # Status rates with real-data defaults
    dns_default = 0.01
    dnf_default = 0.02
    mp_default = 0.01
    dsq_default = 0.005
...
def build_resultlist_xml(event_name, mass_start_dt, teams_data, class_name="Jukolan Viesti"):
    ...
    root.set("creator", "create_artificial_competitors.py")
    ...
    for team in teams_data:
        ...
        for leg_data in team['legs']:
            ...
            # SplitTimes (only for OK runners)
...
def main():
    p = argparse.ArgumentParser(description="Generate synthetic IOF-XML relay competitors")
    ...
    p.add_argument('--teams', type=int, help="Number of teams to generate")
    p.add_argument('--legs', type=int, help="Number of legs per team (1 = individual race, default: all legs from CourseData)")
    p.add_argument('--event-name', help="Event name for the output")
    p.add_argument('--mass-start', help="Mass start time (ISO 8601, e.g. 2025-06-14T23:00:00+03:00)")
    p.add_argument('--speed-mode', choices=['percentile', 'top10', 'manual'],
                   help="Speed generation mode (percentile/top10/manual)")
    ...
    p.add_argument('--non-interactive', action='store_true', help="Skip prompts, use defaults for missing args")
    args = p.parse_args()
    ...
    class_name = cf_els[0].text if cf_els else "Jukolan Viesti"

    # Filter to requested number of legs
    if args.legs is not None:
        if args.legs < 1 or args.legs > n_legs_available:
            print(f"  ERROR: --legs {args.legs} invalid (CourseData has {n_legs_available} legs)")
            sys.exit(1)
        n_legs = args.legs
        courses_by_leg = {leg: courses_by_leg[leg] for leg in range(1, n_legs + 1)}
    else:
        n_legs = n_legs_available

    is_individual = (n_legs == 1)

    print(f"  Loaded {len(flat_names)} courses ({n_legs_available} legs available, using {n_legs}) — {class_name}")
    if is_individual:
    ...
    # Analyze reference results if provided
    ref_stats = None
    if args.ref_results:
    ...
    if args.non_interactive:
        ...
        if not args.event_name:
            if is_individual:
                args.event_name = "Synthetic Individual Orienteering Race"
            elif n_legs == 7:
                args.event_name = "Synthetic Jukolan Viesti"
            else:
                args.event_name = "Synthetic Venla"
        if args.teams is None:
            args.teams = 50
        if not args.mass_start:
        ...
        interactive_setup(courses_by_leg, ref_stats, args, is_individual)
    ...
    if args.seed is not None:
        random.seed(args.seed)  # also set global for generate_splits
    ...
    n_teams = args.teams
    print(f"\n  Generating {n_teams} teams x {n_legs} legs = {n_teams * n_legs} runners...")
    ...
    # Per-leg course assignment: for each leg, pick the most common course
    # (simple: assign all runners to the longest/most common course per leg)
    leg_course_prefs = {}
    for leg in sorted(courses_by_leg.keys()):
        # Prefer the course with the most controls (most "full" course)
        best = max(courses_by_leg[leg].items(),
                   key=lambda item: len(item[1]['controls']))
        leg_course_prefs[leg] = best

    # Pre-compute leg start offsets: for relay mode, leg N starts after cumulative time
    # of previous legs. For individual mode, all runners start at mass_start.
    leg_estimates = {}
    for leg in sorted(courses_by_leg.keys()):
        course_name, course_data = leg_course_prefs[leg]
        km = course_data['length'] / 1000.0
        if ref_stats and leg in ref_stats and ref_stats[leg]['speeds']:
            med_speed = percentile(sorted(ref_stats[leg]['speeds']), 50)
        else:
            med_speed = args.avg_speed or 6.5
        leg_estimates[leg] = (km / med_speed) * 3600 if med_speed > 0 else 3600
    ...
    for bib in range(1, n_teams + 1):
        team_name = generate_team_name(bib, rng)
        team = {'bib': bib, 'team_name': team_name, 'legs': []}
        cumulative_time = 0

        for leg in sorted(courses_by_leg.keys()):
            if is_individual:
                # Individual race: every runner starts at mass_start
                leg_start = mass_start
            else:
                # Relay: start after previous runner's finish
            ...
            # Determine if this is a special status that modifies splits
            is_mp = (status == 'OK' and roll < args.dns_rate + args.dnf_rate + args.mp_rate)
            is_dnf = (status == 'DidNotFinish')

            # Generate splits first (for all non-DNS runners)
            ...
            if is_mp and splits:
                # Remove one random interior control (not first or last)
                if len(splits) > 2:
    ...
    # For individual races (1 leg), position is overall ranking
    for leg in sorted(courses_by_leg.keys()):
    ...
    # Compute overall results (cumulative time, overall position)
    # For individual races, overall = the single leg result
    ...
    # Compute overall positions for teams with all OK legs
    ...
    # Build XML
    mass_start_date = mass_start.strftime('%Y-%m-%d')
    print(f"\n  Writing {args.out}...")
